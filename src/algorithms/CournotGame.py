...
class CournotGame:
    ...
    def compute_equilibrium(self, p_vector, eta, max_iterations=1000, tolerance=0.01):
        ...
        if Sum_C_initial > 0:  # 避免除以零
            ...
            UtilT.print_and_log("警告: Sum_C_initial 非正，无法计算启发式初始值，从零开始。")
        ...
        for iteration in range(max_iterations):
            ...
            if q_previous_norm < 1e-9:  # 调整一个更小的阈值来判断是否“接近零”
                ...
                # UtilT.print_and_log(f"收敛达成 (迭代 {iteration + 1})。相对差异: {diff_norm / q_previous_norm:.2e}")
                break

            if iteration == max_iterations - 1:
                UtilT.print_and_log(f"警告: 达到最大迭代次数 {max_iterations}，可能未完全收敛。")
                current_relative_diff = diff_norm / q_previous_norm if q_previous_norm > 1e-9 else float('inf')
                UtilT.print_and_log(f"最后相对差异: {current_relative_diff:.2e} (绝对差异: {diff_norm:.2e})")
