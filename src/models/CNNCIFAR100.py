...
# 使用CIFAR100数据集，训练cnn
def train_cifar100_cnn(model_save_path="../data/model/cifar100_cnn"):
    """
    使用CIFAR100数据集，训练cnn
    ...
    train_batch_file = "../data/dataset/CIFAR100/train"
    test_batch_file = "../data/dataset/CIFAR100/test"

    # 加载训练数据和测试数据
    train_dataset = CIFAR100Dataset(batch_files=[train_batch_file], transform=transform)
    test_dataset = CIFAR100Dataset(batch_files=[test_batch_file], transform=transform)
...
# 修改微调函数逻辑
def fine_tune_cifar100_cnn(parameters, train_loader, num_epochs=5, device='cpu', lr=1e-5):
    """
    微调模型参数
