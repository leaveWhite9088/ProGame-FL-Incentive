"""
tiflcs-marp-accuracy-CIFAR100.py
基于 PGI-RDFL 修改的固定 Eta 和排名选择模型 - 精度实验 (CIFAR-100 版本)
...
def init_model(rate):
    ...
    model = CIFAR100CNN(num_classes=100).to(device) # <-- 更改为 CIFAR100CNN
    ...
    model_save_path = f"{project_root}/data/model/initial/cifar100_cnn_initial_model" # <-- 更改
    model_path = f"{project_root}/data/model/cifar100_cnn_model" # <-- 更改
...
def evaluate_data_quality(dataowners):
    ...
    for i, do in enumerate(dataowners):
        ...
        if not mse_scores:
            UtilCIFAR100.print_and_log(parent_path, f"DataOwner{i+1} 没有数据，跳过质量评估。")
...
def calculate_fixed_eta_and_ranked_selection(avg_f_list, last_xn_list, N, Rho_val, Lambda_val, fixed_eta,
                                             num_to_select):
    """
    使用固定Eta，并根据综合得分排名选择Top-K客户端 (CIFAR100 版本)
    ...
    # 2. 将q_star转化为x_opt (潜在数据量)，使用 UtilCIFAR100
    ...
    UtilCIFAR100.print_and_log(parent_path, f"[Fixed Eta Model] 选择了 {k} 个客户端 (Top-{num_to_select})")
    UtilCIFAR100.print_and_log(parent_path, f"[Fixed Eta Model] 客户端索引: {top_k_indices}")

    # 5. 确定最终数据量，使用 UtilCIFAR100
    xn_list = UtilCIFAR100.compare_elements(x_opt, last_xn_list)
    ...
    UtilCIFAR100.print_and_log(parent_path, f"[Fixed Eta Model] 固定总支付η: {best_Eta:.4f}")
    UtilCIFAR100.print_and_log(parent_path, f"[Fixed Eta Model] 领导者效用: {leader_utility:.4f}")
    UtilCIFAR100.print_and_log(parent_path, f"[Fixed Eta Model] 跟随者平均效用: {avg_follower_utility:.4f}")
...
def compute_contribution_rates(xn_list, avg_f_list, pn_list, best_Eta, N):
    ...
    UtilCIFAR100.print_and_log(parent_path, f"ModelOwner的总支付：{best_Eta}")
    if sum_qn == 0:
        UtilCIFAR100.print_and_log(parent_path, "总贡献为0，没有支付分配。")
        return

    for i in range(N):  # 遍历所有N个客户端
        if pn_list[i] == 0:  # 如果客户端未参与
            UtilCIFAR100.print_and_log(parent_path, f"DataOwner{i + 1}: 未参与 (pn=0)")
            continue
...
def submit_data_to_cpc(matching, dataowners, ComputingCenters, xn_list, pn_list):
    ...
    for item in matching.items():
        ...
        if pn_list[dataowner_index] == 0:
            UtilCIFAR100.print_and_log(parent_path, f"DataOwner{dataowner_index + 1} 未被选中，不提交数据。")
            continue
...
def fine_tune_model(cpcs, matching, test_loader, lr=1e-5, device='cpu', num_epochs=5, force_update=False,
    ...
    # 1. 创建并加载CNN模型
    model = CIFAR100CNN(num_classes=100).to(device) # <-- 更改
    ...
    for item in matching.items():
        ...
        UtilCIFAR100.print_and_log(parent_path, f"{item[1]}调整模型中, 本轮训练的数据量为：{len(cpcs[cpc_index].imgData) :.2f} :") # <-- 更改
        if len(cpcs[cpc_index].imgData) == 0:
            UtilCIFAR100.print_and_log(parent_path, "数据量为0，跳过此轮调整") # <-- 更改
            continue

        train_loader = UtilCIFAR100.create_data_loader(cpcs[cpc_index].imgData, cpcs[cpc_index].labelData, batch_size=64, # <-- 更改
                                                    shuffle=True)

        UtilCIFAR100.print_and_log(parent_path, "开始本地模型训练...") # <-- 更改
        updated_params = fine_tune_cifar100_cnn( # <-- 更改
    ...
    # 4. 上传参数
    UtilCIFAR100.print_and_log(parent_path, "本地训练完成，参数已准备好进行聚合") # <-- 更改

    # 5. 合并参数
    # 【注意】: 按照你的模板
    if not updated_params_list:
        UtilCIFAR100.print_and_log(parent_path, "没有客户端提交模型参数，跳过聚合和更新。") # <-- 更改

        # 假设 CIFAR100CNN 也有一个 .evaluate_model 方法
    ...
    UtilCIFAR100.print_and_log(parent_path, "参数聚合完成") # <-- 更改

    # 6. 选择更新
    UtilCIFAR100.print_and_log(parent_path, "评估聚合后的模型参数...") # <-- 更改
    ...
    UtilCIFAR100.print_and_log(parent_path, "模型更新流程完成") # <-- 更改
    return new_accuracy


if __name__ == "__main__":
    UtilCIFAR100.print_and_log( # <-- 更改
        parent_path,
        f"**** {parent_path} (Fixed Eta & Ranked Selection - CIFAR100) 运行时间： {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ****")
    ...
    for n in [9]:
        UtilCIFAR100.print_and_log(parent_path, f"========================= 客户端总数 N: {n + 1} =========================") # <-- 更改
        UtilCIFAR100.print_and_log(parent_path, f"========================= 固定支付 ETA: {FIXED_ETA} =========================") # <-- 更改
        UtilCIFAR100.print_and_log(parent_path, f"========================= 选择数量 K: {NUM_TO_SELECT} =========================") # <-- 更改

        UtilCIFAR100.print_and_log(parent_path, "---------------------------------- 定义参数值 ----------------------------------") # <-- 更改
        ...
        UtilCIFAR100.print_and_log(parent_path, "DONE") # <-- 更改

        UtilCIFAR100.print_and_log(parent_path, "---------------------------------- 准备工作 ----------------------------------") # <-- 更改
        ...
        while literation < num_iterations:
            ...
            if literation == 0:
                UtilCIFAR100.print_and_log(parent_path, f"----- literation {literation + 1}: 为 DataOwner 的数据添加噪声 -----") # <-- 更改
                dataowner_add_noise(dataowners, 0.1)
                UtilCIFAR100.print_and_log(parent_path, "DONE") # <-- 更改

                UtilCIFAR100.print_and_log(parent_path, f"----- literation {literation + 1}: 计算 DataOwner 的数据质量 -----") # <-- 更改
                avg_f_list = evaluate_data_quality(dataowners)  # 移除 N
                UtilCIFAR100.print_and_log(parent_path, "DONE") # <-- 更改
            ...
            UtilCIFAR100.print_and_log(parent_path, f"----- literation {literation + 1}: DataOwner 分配 ModelOwner 的支付 -----") # <-- 更改
            compute_contribution_rates(xn_list, avg_f_list, pn_list, best_Eta, N)
            UtilCIFAR100.print_and_log(parent_path, "DONE") # <-- 更改

            # 匹配只在第一轮进行
            if literation == 0:
                UtilCIFAR100.print_and_log(parent_path, f"----- literation {literation + 1}: 匹配 DataOwner 和 ComputingCenter -----") # <-- 更改
                matching = match_data_owners_to_cpc(xn_list, ComputingCenters, dataowners, SigmaM, N, Rho_val)
                UtilCIFAR100.print_and_log(parent_path, "DONE") # <-- 更改

            UtilCIFAR100.print_and_log(parent_path, f"----- literation {literation + 1}: DataOwner 向 ComputingCenter 提交数据 -----") # <-- 更改
            submit_data_to_cpc(matching, dataowners, ComputingCenters, xn_list, pn_list)
            UtilCIFAR100.print_and_log(parent_path, "DONE") # <-- 更改

            UtilCIFAR100.print_and_log(parent_path, f"----- literation {literation + 1}: 模型训练 -----") # <-- 更改
            ...
            UtilCIFAR100.print_and_log(parent_path, f"[记录精度] 第{literation + 1}轮精度: {new_accuracy:.4f}") # <-- 更改
            UtilCIFAR100.print_and_log(parent_path, f"accuracy_list: {accuracy_list}") # <-- 更改
            UtilCIFAR100.print_and_log(parent_path, "DONE") # <-- 更改
    ...
    UtilCIFAR100.print_and_log(parent_path, "\n===== (Fixed Eta & Ranked Selection - CIFAR100) 实验最终结果 =====") # <-- 更改
    UtilCIFAR100.print_and_log(parent_path, f"accuracy_list_total: {accuracy_list_total}") # <-- 更改

    # 假设只跑了一组n (n=9)
    if accuracy_list_total:
        final_run_accuracy = accuracy_list_total[0]
        if final_run_accuracy:
            UtilCIFAR100.print_and_log(parent_path, f"最终精度: {final_run_accuracy[-1]:.4f}") # <-- 更改
            UtilCIFAR100.print_and_log(parent_path, f"平均精度: {np.mean(final_run_accuracy):.4f}") # <-- 更改
            if len(final_run_accuracy) > 1:
                UtilCIFAR100.print_and_log(parent_path, f"精度提升: {(final_run_accuracy[-1] - final_run_accuracy[0]):.4f}") # <-- 更改
            else:
                UtilCIFAR100.print_and_log(parent_path, f"精度提升: 0.0000 (仅一轮)") # <-- 更改
        else:
            UtilCIFAR100.print_and_log(parent_path, "未记录任何精度。") # <-- 更改