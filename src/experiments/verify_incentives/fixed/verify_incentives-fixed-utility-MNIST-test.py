...
def init_model(rate):
    ...
    # 直接调用新函数加载分层子集，rate参数决定了抽样比例
    train_images, train_labels = UtilMNIST.load_and_create_stratified_subset(train_images_path, train_labels_path, fraction=rate)

    # 【注意】下面的随机采样代码块已被完全替代并删除
    ...
    model_save_path = f"{project_root}/data/model/initial/mnist_cnn_initial_model_fraction_{rate}" # 建议根据比例保存不同模型
...
def calculate_optimal_payment_and_data(avg_f_list, last_xn_list):
    ...
    p_star_optimal, eta_star_optimal, q_star_optimal, _, _ = stackelberg_solver.solve()

    # 设置固定的 eta，这里动态调整以确保合理性
    # 基于最优 eta 的一定比例，确保不会过高
    fix_Eta = min(1.0, eta_star_optimal * 0.8)  # 使用最优值的80%作为固定值

    from src.algorithms.CournotGame import CournotGame
    cournot_solver = CournotGame(N, Rho * Lambda, avg_f_list, Lambda)
    q_star_fixed = cournot_solver.compute_equilibrium(p_star_optimal, fix_Eta)

    # 计算固定 eta 下的领导者效用
    S_star_fixed = np.sum(p_star_optimal * q_star_fixed)
    leader_utility_fixed = S_star_fixed - fix_Eta

    # 如果 leader_utility 仍然为负，进一步调整
    if leader_utility_fixed < 0:
        # 将 fix_Eta 设置为能确保非负效用的值
        fix_Eta = S_star_fixed * 0.9  # 保留10%作为正效用
        leader_utility_fixed = S_star_fixed - fix_Eta

    # 计算跟随者效用
    if S_star_fixed > 1e-9:
        price_at_equilibrium = fix_Eta / S_star_fixed
        follower_utilities_fixed = q_star_fixed * (price_at_equilibrium - Rho * Lambda)
    else:
        follower_utilities_fixed = np.zeros(N)

    # 将q_star转化为x_opt
    x_opt = UtilMNIST.power_transform_then_min_max_normalize(q_star_fixed)

    # 将pn_list(p_star)做归一化
    p_star = UtilMNIST.power_transform_then_min_max_normalize(p_star_optimal)

    UtilMNIST.print_and_log(f"fix_Eta：{fix_Eta}")

    return UtilMNIST.compare_elements(x_opt,
                                      last_xn_list), p_star, fix_Eta, leader_utility_fixed, np.sum(follower_utilities_fixed) / N
...
if __name__ == "__main__":
    ...
    UtilMNIST.print_and_log("fixed 最终的列表：")
    UtilMNIST.print_and_log(f"U_qn_list: {U_qn_list}")
    UtilMNIST.print_and_log(f"U_Eta_list: {U_Eta_list}")
