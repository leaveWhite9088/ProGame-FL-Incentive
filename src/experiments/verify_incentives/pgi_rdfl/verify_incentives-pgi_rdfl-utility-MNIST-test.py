...
def calculate_optimal_payment_and_data(avg_f_list, last_xn_list):
    ...
    #
    # # 保存原始的优化结果用于计算U_Eta
    # original_p_star = p_star.copy()
    # original_q_star = q_star.copy()
    #
    # # 将q_star转化为x_opt
    ...
    # p_star_normalized = UtilMNIST.power_transform_then_min_max_normalize(p_star)
    #
    # # 重新计算基于归一化后值的eta，以保证U_Eta非负
    # # 调整eta使得 U_Eta = Σ(p_n * q_n) - eta >= 0
    # # 将list转换为numpy array以进行元素级乘法
    # p_star_normalized_np = np.array(p_star_normalized)
    # x_opt_np = np.array(x_opt)
    # sum_pq_normalized = np.sum(p_star_normalized_np * x_opt_np)
    # eta_adjusted = min(eta_star, sum_pq_normalized * 0.9)  # 保留10%的余量确保U_Eta为正
    #
    # # 使用原始值计算真实的U_Eta，但返回调整后的值给后续使用
    # leader_utility_adjusted = sum_pq_normalized - eta_adjusted
    #
    # return UtilMNIST.compare_elements(x_opt, last_xn_list), p_star_normalized, eta_star, leader_utility_adjusted, follower_utilities / N
