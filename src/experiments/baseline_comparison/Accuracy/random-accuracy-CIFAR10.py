...
# ModelOwner计算模型总体支付，DataOwner确定提供的最优数据量 (random方法版本)
def calculate_optimal_payment_and_data(avg_f_list, last_xn_list, N, Rho_val, Lambda_val):
    ...
    # 处理p_star全为0的情况，确保每个DataOwner都有一定的选择概率
    if np.sum(p_star) <= 1e-9:
        p_star = np.ones(N) / N
    else:
        # 确保p_star归一化，总和为1
    ...
    if sum_qn == 0:
        ...
        for i in range(len(xn_list)):
            ...
            UtilCIFAR10.print_and_log(parent_path,
                f"pn:{pn_list[i]}; xn:{xn_list[i]}; 分配到的支付：0.0000")
        return
...
def fine_tune_model(cpcs, matching, test_loader, lr=1e-5, device='cpu', num_epochs=5, force_update=False,
    ...
    if len(updated_params_list) == 0:
        UtilCIFAR10.print_and_log(parent_path, "警告: 没有更新的参数，返回原始模型的准确率")
