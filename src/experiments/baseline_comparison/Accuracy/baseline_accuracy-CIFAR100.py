...
def calculate_baseline_payment_and_data(avg_f_list, last_xn_list, N, Rho_val, Lambda_val):
    ...
    UtilCIFAR100.print_and_log(parent_path, f"[基线模型] 固定总支付η: {eta_star:.4f}")
    UtilCIFAR100.print_and_log(parent_path, f"[基线模型] 领导者效用: {leader_utility:.4f}")
    UtilCIFAR100.print_and_log(parent_path, f"[基线模型] 跟随者平均效用: {avg_follower_utility:.4f}")
...
if __name__ == "__main__":
    UtilCIFAR100.print_and_log(parent_path, f"**** Baseline Two-Party Stackelberg-Cournot - Accuracy Experiment ****")
    UtilCIFAR100.print_and_log(parent_path, f"**** 基线两方斯塔克尔伯格-古诺模型 - 精度实验 ****")
    UtilCIFAR100.print_and_log(parent_path, f"**** 运行时间： {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ****")
    ...
    for n in [9]:  # 10个客户端
        ...
        while literation < num_iterations:
            ...
            # 使用基线模型计算支付和数据量
            UtilCIFAR100.print_and_log(parent_path, f"----- literation {literation + 1}: [基线模型] 计算 ModelOwner 总体支付和 DataOwners 最优数据量 -----")
            UtilCIFAR100.print_and_log(parent_path, "[基线特征] p固定为全1，只优化η")
            ...
            UtilCIFAR100.print_and_log(parent_path, f"[记录精度] 第{literation + 1}轮精度: {new_accuracy:.4f}")
    ...
    UtilCIFAR100.print_and_log(parent_path, f"各轮精度 accuracy_list_total: {accuracy_list_total}")
    UtilCIFAR100.print_and_log(parent_path, f"最终精度: {accuracy_list_total[-1]:.4f}")
    UtilCIFAR100.print_and_log(parent_path, f"平均精度: {np.mean(accuracy_list_total):.4f}")
    UtilCIFAR100.print_and_log(parent_path, f"精度提升: {(accuracy_list_total[-1] - accuracy_list_total[0]):.4f}")
