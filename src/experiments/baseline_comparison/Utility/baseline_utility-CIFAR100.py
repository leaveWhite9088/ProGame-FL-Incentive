...
if __name__ == "__main__":
    UtilsCIFAR100.print_and_log(parent_path, f"**** Baseline Two-Party Stackelberg-Cournot - Utility Experiment ****")
    UtilsCIFAR100.print_and_log(parent_path, f"**** 基线两方斯塔克尔伯格-古诺模型 - 效用实验 ****")
    ...
    for n in [9, 19, 29, 39, 49, 59, 69, 79, 89, 99]:
        ...
        while True:
            ...
            if literation == adjustment_lit + 1:
                ...
                UtilsCIFAR100.print_and_log(parent_path, f"[记录效用] U_Eta: {U_Eta:.4f}, U_qn: {U_qn:.4f}")
            
            UtilsCIFAR100.print_and_log(parent_path, "DONE")
            
            # 提前中止（只关注效用值）
            if literation > adjustment_lit:
                UtilsCIFAR100.print_and_log(parent_path, f"完成效用实验，准备输出结果...")
                break
    ...
    # 输出最终结果
    UtilsCIFAR100.print_and_log(parent_path, "\n===== 基线效用实验最终结果 =====")
    UtilsCIFAR100.print_and_log(parent_path, f"模型拥有者效用 U_Eta_list: {U_Eta_list}")
    UtilsCIFAR100.print_and_log(parent_path, f"数据拥有者平均效用 U_qn_list: {U_qn_list}")
