...
def ready_for_task(rate):
    ...
    train_data_dir = f"{project_root}/data/dataset/CIFAR100"
    test_data_dir = f"{project_root}/data/dataset/CIFAR100"

    # 加载训练数据和测试数据
    train_images, train_labels, test_images, test_labels = UtilCIFAR100.load_cifar100_dataset(train_data_dir)

    # 创建DataOwner对象数组
    dataowners = [DataOwner(Lambda=Lambda, Rho=Rho) for _ in range(N)]  # 假设有5个DataOwner

    # 切分数据
    UtilCIFAR100.split_data_to_dataowners_with_large_gap(dataowners, train_images, train_labels)
...
def init_model(rate):
    ...
    data_dir = f"{project_root}/data/dataset/CIFAR100"

    # 加载训练数据
    train_images, train_labels, _, _ = UtilCIFAR100.load_cifar100_dataset(data_dir)
    ...
    train_loader = UtilCIFAR100.create_data_loader(train_images, train_labels, batch_size=64, shuffle=True, num_workers=0)
    ...
    if os.path.exists(model_save_path):
        UtilCIFAR100.print_and_log(parent_path, f"{model_save_path} 存在，加载初始化模型")
        model.load_model(model_save_path)
        model.save_model(f"{project_root}/data/model/cifar100_cnn_model")
    else:
        UtilCIFAR100.print_and_log(parent_path, f"{model_save_path} 不存在，初始化模型")
...
if __name__ == "__main__":
    ...
    for n in [9, 19, 29, 39, 49, 59, 69, 79, 89, 99]:
        ...
        while True:
            ...
            # 提前中止
            if literation > adjustment_literation:
                UtilCIFAR100.print_and_log(parent_path, f"U_qn_list: {U_qn_list}")
                UtilCIFAR100.print_and_log(parent_path, f"U_Eta_list: {U_Eta_list}")
                break
    ...
    UtilCIFAR100.print_and_log(parent_path, "random 最终的列表：")
    UtilCIFAR100.print_and_log(parent_path, f"U_qn_list: {U_qn_list}")
    UtilCIFAR100.print_and_log(parent_path, f"U_Eta_list: {U_Eta_list}")