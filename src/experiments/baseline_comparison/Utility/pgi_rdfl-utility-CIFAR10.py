"""
PGI-RDFL模型 - 效用实验
PGI-RDFL Model - Utility Experiment

本实验实现了PGI-RDFL（Proactive Game-based Incentive Mechanism with Reward and Dynamic Feedback Learning）模型：
- 模型拥有者（领导者）同时优化总支付η和选择概率p
...
def calculate_optimal_payment_and_data(avg_f_list, last_xn_list, N, Rho_val, Lambda_val):
    ...
    x_opt = UtilsCIFAR10.power_transform_then_min_max_normalize(q_star)

    # 将pn_list(p_star)做归一化
    p_star = UtilsCIFAR10.power_transform_then_min_max_normalize(p_star)

    return UtilsCIFAR10.compare_elements(x_opt, last_xn_list), p_star, eta_star, leader_utility, follower_utilities / N
...
if __name__ == "__main__":
    UtilsCIFAR10.print_and_log(parent_path, f"**** PGI-RDFL - Utility Experiment ****")
    ...
    for n in [9, 19, 29, 39, 49, 59, 69, 79, 89, 99]:
        ...
        while True:
            ...
            if literation > adjustment_lit:
                UtilsCIFAR10.print_and_log(parent_path, f"U_qn_list: {U_qn_list}")
                UtilsCIFAR10.print_and_log(parent_path, f"U_Eta_list: {U_Eta_list}")
                break
    ...
    UtilsCIFAR10.print_and_log(parent_path, "pgirdfl 最终的列表：")
    UtilsCIFAR10.print_and_log(parent_path, f"U_qn_list: {U_qn_list}")
    UtilsCIFAR10.print_and_log(parent_path, f"U_Eta_list: {U_Eta_list}")